...
class CellRankTableViewCell: UITableViewCell {
    ...
    private func updateView() {
        ...
        let circle = CircleGraphic(frame: CGRect(x: xStart,
                                   ...
                                   percent: Int(viewModel.rateActive), title: App.TitleInChart.active, color: #colorLiteral(red: 0.262745098, green: 0.6352941176, blue: 0.7058823529, alpha: 1))
        circle.tag = 1
        addSubview(circle)
        ...
        let circle1 = CircleGraphic(frame: CGRect(x: xStart + widthCircle1 + spaceChart,
                                    ...
                                    percent: Int(viewModel.rateRecover), title: App.TitleInChart.recovered, color: #colorLiteral(red: 1, green: 0.8196078431, blue: 0.831372549, alpha: 1))
        circle1.tag = 1
        addSubview(circle1)
        ...
        let circle2 = CircleGraphic(frame: CGRect(x: xStart + 2 * widthCircle2 + 2 * spaceChart,
                                    ...
                                    percent: Int(viewModel.rateDeadth), title: App.TitleInChart.deadth, color: #colorLiteral(red: 0.2156862745, green: 0.1882352941, blue: 0.3803921569, alpha: 1))
        circle2.tag = 1
        addSubview(circle2)
    }
}
